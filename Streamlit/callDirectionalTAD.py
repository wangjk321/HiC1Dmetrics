...
def TADcallIS(matrixPath,resolution,chromosome,squareSize=150000):
    ISbedgraph = InsulationScore(matrixPath,resolution,chromosome,square_size=squareSize).getScore()
...
class DirectionalTAD(DiffDraw):
    def extractRegion(self,dataControl):
        ControlTad = TADcallIS(dataControl,self.resolution,self.chr,squareSize= self.sizeIS)
    ...
    def plotAlldirec(self,type,dataControl,dataTreat):
        if type == "left":
            region,_,drf = self.extractRegion(dataControl)
        elif type == "right":
            _,region,drf = self.extractRegion(dataControl)
        else:
        ...
        for i in range(num):
            ...
            PlotTri(dataControl,self.resolution,startSite=start,endSite=end,
            ...
            PlotTri(dataTreat,self.resolution,startSite=start,endSite=end,
